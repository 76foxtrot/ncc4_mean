...
function findAllHours (callback) {
    Students.find({ }, function (err, data) {
        if (err) { console.log(err); return undefined;}
...
function findOne(id, callback) {
    ...
    Students.find({ _id: oid }, function(err, student) {
        if(err) { console.log('student ' + id + ' not found'); }
        if (callback) { callback(student); }
    });
};
function findOnesHours(id, date, callback) {
    var oid = mongoose.Types.ObjectId(id);
...
function addOne(student, callback) {
...
function updateOne(id, data, callback) {
    var oid = mongoose.Types.ObjectId(id);
    var options = { new: true, upsert: false};
    Students.findOneAndUpdate({ _id: oid }, data, options, function(err, student) {
        var ret = undefined;
        if(err) { console.log('student ' + id + ' not found'); }
        else { ret = {}; }
...
function deleteOne(id, callback) {
    var oid = mongoose.Types.ObjectId(id);
    Students.findOneAndRemove({ _id: oid }, function(err, student) {
        var ret = undefined;
        if(err) { console.log('student ' + id + ' not found'); }
        else { ret = {}; }
        if (callback) { callback(ret); }
...
function addHours(id, hours, callback) {
    var oid = mongoose.Types.ObjectId(id);
    Students.update(
        { _id: oid },
        { $push: { hours: hours } },
